...
export default function AdminOrdersPanel() {
	...
	const confirmArchive = async () => {
		...
		try {
			...
			setOrders((prev) =>
				prev.filter((order) => order._id !== deleteModalOrder._id),
			);
						...
						{loading ? (
							...
							orders.map((order) => {
								const selectedStatus =
									selectedStatusByOrderId[order._id] || order.status;
										...
										<td className="py-3 pr-2">
											...
											<div>
												<select
													...
													disabled={
														statusLocked || updatingOrderId === order._id
													}
													onChange={(event) =>
														setSelectedStatusByOrderId((prev) => ({
															...prev,
