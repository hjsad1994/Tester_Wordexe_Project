...
export default function AdminProductsPanel({
	...
	const [deletingProduct, setDeletingProduct] = useState<ApiProduct | null>(
		null,
	);
	...
	const previewImageUrl =
		...
		(removeExistingImage ? null : editingProductImage);
		...
		try {
			...
			if (editingProductId) {
				...
				if (selectedImageFile) {
					const uploaded = await uploadProductImageApi(
						editingProductId,
						selectedImageFile,
					);
	...
	const confirmDeleteProduct = async () => {
		...
		try {
			...
			setAdminProducts((prev) =>
				prev.filter((product) => product._id !== deletingProduct._id),
			);
	...
	const getCategoryName = (product: ApiProduct) => {
		...
		return (
			adminCategories.find((category) => category._id === product.category)
				?.name || "Không xác định"
		);
	};
					...
					<thead>
						<tr className="border-b border-pink-100 text-left text-[var(--text-muted)]">
							...
							<th className="py-2 pr-2">
								{isInventoryView ? "Danh mục" : "Giá"}
							</th>
							<th className="py-2 pr-2">Tồn kho</th>
							<th className="py-2 pr-2">
								{isInventoryView ? "Mức tồn" : "Danh mục"}
							</th>
							<th className="py-2 pr-2">
								{isInventoryView ? "Khuyến nghị" : "Trạng thái"}
							</th>
						...
						{adminLoading ? (
							...
							displayedProducts.map((product) => {
								...
								return (
									<tr
										key={product._id}
										className="align-top border-b border-pink-50"
									>
										...
										<td className="py-3 pr-2">
											{isInventoryView ? (
												<span className={stockLevel.badgeClass}>
													{stockLevel.label}
												</span>
											) : (
												getCategoryName(product)
									...
									<div className="grid gap-3 rounded-2xl border border-pink-100 bg-pink-50/40 p-4 text-sm text-[var(--text-muted)] sm:grid-cols-2">
										...
										<div>
											...
											<p className="mt-1 font-medium text-[var(--text-primary)]">
												{editingProduct
													? getCategoryName(editingProduct)
													: "Không xác định"}
											</p>
										</div>
									</div>
									...
									<div className="rounded-2xl border border-pink-100 bg-pink-50/40 p-4">
										...
										{previewImageUrl ? (
											...
											<p className="text-xs text-[var(--text-muted)]">
												Chưa chọn ảnh mới. Bạn có thể thay ảnh hiện tại hoặc giữ
												nguyên khi lưu.
											</p>
										)}
									</div>
