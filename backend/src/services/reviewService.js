...
class ReviewService {
		...
		try {
			...
			for (const image of images) {
				try {
					...
					console.error(
						`Failed to cleanup Cloudinary image: ${image.publicId}`,
					);
				}
			}
			throw error;
	...
	async getReviews(productId, options = {}, requestingUserId = null) {
		...
		const distribution = await reviewRepository.getRatingDistribution(
			product._id,
		);
	...
	async toggleHelpful(reviewId, userId) {
		...
		const alreadyLiked = review.helpfulBy.some(
		...
		if (!updated) {
			...
			return {
				...
				isLiked: current.helpfulBy.some(
					(id) => id.toString() === userId.toString(),
				),
			};
		}
