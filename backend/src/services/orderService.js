...
class OrderService {
		...
		try {
			const order = await Order.create({
				...
				total:
					subtotal -
					discountAmount +
					(payload.couponCode ? finalShippingFee : shippingFee),
				customerInfo,
				paymentMethod,
				status,
				statusHistory: [
	...
	async listOrders(options = {}) {
		...
		const limit =
			Number(options.limit) > 0 ? Math.min(Number(options.limit), 100) : 20;
	...
	async updateOrderStatus(id, nextStatus, context = {}) {
		...
		if (order.status === "delivered") {
			throw new ValidationError(
				"Delivered orders are locked and cannot change status",
			);
		}
